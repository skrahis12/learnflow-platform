...
const CourseDetail = () => {
    ...
    if (course) {
      try {
        const historyItem = {
          ...
          lastLesson: {
            ...(activeLesson || {}),
            videoUrl: activeLesson?.videoUrl || course.videoUrl || ""
  ...
  }, [activeLesson, course, currentVideoUrl]);
          ...
          <div className="lg:col-span-1">
            <div className="bg-card rounded-2xl overflow-hidden shadow-xl border border-border sticky top-24">
              <div className="aspect-video relative bg-black rounded-t-2xl overflow-hidden">
                {showPreview && (currentVideoUrl || courseData.videoUrl) ? (
                  <iframe
                    width="100%"
                    height="100%"
                    src={`${currentVideoUrl || courseData.videoUrl}${String(currentVideoUrl || courseData.videoUrl).includes('?') ? '&' : '?'}autoplay=1&rel=0&modestbranding=1`}
                  ...
                  <div className="absolute inset-0 flex items-center justify-center bg-black/80 flex-col">
                     <span className="text-white font-semibold">Video preview unavailable</span>
                     <span className="text-white/60 text-sm mt-2 font-light">The instructor didn't upload a video yet.</span>
                  </div>
                ) : (
                  <>
                ...
                {courseData.modules.map((module, moduleIndex) => (<AccordionItem key={moduleIndex} value={`module-${moduleIndex}`} className="bg-card border border-border rounded-xl overflow-hidden">
                  ...
                  <AccordionContent className="px-0 pb-0">
                    {module.lessons.map((lesson, lessonIndex) => (<div key={lessonIndex}
                      onClick={() => {
                        if (lesson.preview) { // Allow clicking all for tracking demo, usually check permissions
                          setActiveLesson({
                            title: lesson.title,
